...
async function fetchMaintenanceInfo(craneId) {
  try {
    ...
    if (craneDetail.breakdowns && craneDetail.breakdowns.length > 0) {
      const latestLog = craneDetail.breakdowns[0]; // Assuming logs are ordered by date
...
async function saveNewCrane() {
  ...
  if (status === 1) {
    ...
    formData.append('Breakdown.Reasons', reasons);
  }
